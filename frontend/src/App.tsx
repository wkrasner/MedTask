...
function TaskDetail({ task, customTypes, currentUser, onClose, onEdit, onStatusChange, onAddActivity }: {
  task: BaseTask
  customTypes: CustomTaskTypeDef[]
  currentUser: { staffId: string; staffName: string }
  onClose: () => void
  onEdit: () => void
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          ...
          <div style={{ padding: '16px 20px' }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#9CA3AF', letterSpacing: '0.06em', marginBottom: 12 }}>
              ACTIVITY LOG {log.length > 0 && <span style={{ background: '#F3F4F6', borderRadius: 10, padding: '1px 6px', marginLeft: 4, color: '#6B7280', fontSize: 10 }}>{log.length}</span>}
            </div>

            {/* Existing entries */}
            {log.length === 0 ? (
              <div style={{ fontSize: 12, color: '#D1D5DB', textAlign: 'center', padding: '16px 0', borderTop: '1px dashed #E5E7EB' }}>No activity yet — add the first note below</div>
            ) : (
            ...
            <div style={{ marginTop: 14, borderTop: '1px solid #F3F4F6', paddingTop: 14 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: '#9CA3AF', letterSpacing: '0.06em', marginBottom: 8 }}>ADD NOTE AS {currentUser.staffName.toUpperCase()}</div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                ...
                />
                <button onClick={handleAddNote} disabled={!newNote.trim()} style={{ padding: '8px 14px', borderRadius: 8, border: 'none', background: newNote.trim() ? 'linear-gradient(135deg,#6366F1,#8B5CF6)' : '#E5E7EB', color: newNote.trim() ? '#fff' : '#9CA3AF', cursor: newNote.trim() ? 'pointer' : 'not-allowed', fontSize: 12, fontWeight: 700, fontFamily: 'inherit', whiteSpace: 'nowrap', alignSelf: 'flex-end' }}>
                  Add note
                </button>
              </div>
            </div>
...
function Dashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const handleAddActivity = (taskId: string, entry: ActivityEntry) => {
    ...
    // Keep selected task in sync so the modal updates immediately
    setSelected(prev => prev && prev.taskId === taskId
      ? { ...prev, activityLog: [...(prev.activityLog ?? []), entry], updatedAt: new Date().toISOString() }
      : prev
    )
  }

  // Derive current user from auth — fall back to first staff member
  const currentUser = { staffId: 's1', staffName: 'Staff Member' }
